...
#define kzalloc(size, flags)                                                   \
  memset(heap_caps_malloc(size, MALLOC_CAP_8BIT), 0, size)
...
static inline unsigned int gf_mul(struct bch_control *bch, unsigned int a,
                                  unsigned int b) {
  return (a && b)
             ? bch->a_pow_tab[mod_s(bch, bch->a_log_tab[a] + bch->a_log_tab[b])]
             : 0;
}
...
static uint32_t *compute_generator_polynomial(struct bch_control *bch) {
  ...
  if (err) {
    ESP_LOGE(TAG, "error in compute_generator_polynomial for roots %u",
             (bch->n + 1) * sizeof(*roots));
  }
  genpoly = bch_alloc(DIV_ROUND_UP(m * t + 1, 32) * sizeof(*genpoly), &err);

  if (err) {
    ESP_LOGE(TAG, "error in compute_generator_polynomial for genpoly %u",
             DIV_ROUND_UP(m * t + 1, 32) * sizeof(*genpoly));
    kfree(genpoly);
    genpoly = NULL;
    goto finish;
  }
