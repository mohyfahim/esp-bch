...
#define GFP_KERNEL
#define kmalloc(size, flags) heap_caps_malloc(size, MALLOC_CAP_8BIT)
#define kzalloc(size, flags) memset(heap_caps_malloc(size, MALLOC_CAP_8BIT), 0, size)
#define kfree(ptr) heap_caps_free(ptr)
...
static uint32_t *compute_generator_polynomial(struct bch_control *bch)
{
	...
	if (err)
	{
		ESP_LOGE(TAG, "error in compute_generator_polynomial for g");
	}
	// ESP_LOGW(TAG, "all : %d and free : %d",
	// 		 heap_caps_get_total_size(MALLOC_CAP_8BIT), heap_caps_get_free_size(MALLOC_CAP_8BIT));
	// heap_caps_print_heap_info(MALLOC_CAP_8BIT);
	roots = bch_alloc((bch->n + 1) * sizeof(*roots), &err);
	if (err)
	{
		ESP_LOGE(TAG, "error in compute_generator_polynomial for roots %u", (bch->n + 1) * sizeof(*roots));
	}
	genpoly = bch_alloc(DIV_ROUND_UP(m * t + 1, 32) * sizeof(*genpoly), &err);

	if (err)
	{
		ESP_LOGE(TAG, "error in compute_generator_polynomial for genpoly %u", DIV_ROUND_UP(m * t + 1, 32) * sizeof(*genpoly));
		kfree(genpoly);
		genpoly = NULL;
		goto finish;
	}
...
struct bch_control *init_bch(int m, int t, unsigned int prim_poly)
{
	...
	ESP_LOGW(TAG, "size of buff %u", words * sizeof(*bch->ecc_buf));
	ESP_LOGW(TAG, "size of buff2 %u", words * sizeof(*bch->ecc_buf2));
	...
	if (err)
	{
		ESP_LOGE(TAG, "error on bc_alloc with GF_POLY_SZ");
		goto fail;
	}
	...
	if (err)
	{
		ESP_LOGE(TAG, "error on build_gf_tables");
		goto fail;
	}
	...
	if (genpoly == NULL)
	{
		ESP_LOGE(TAG, "error on compute_generator_polynomial");
	...
	if (err)
	{
		ESP_LOGE(TAG, "error on build_deg2_base");
		goto fail;
	}
	return bch;

fail:
	ESP_LOGE(TAG, " error in initialization of bch");
	free_bch(bch);
	return NULL;
}
